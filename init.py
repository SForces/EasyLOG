...
def LogFile_path(path: str):
    ...
    if os.path.exists(formatted_path):
        ...
        with open(formatted_path, "w") as f:
            pass
        with open("EasyLOG/path.ini", "w") as f:
            f.write(formatted_path)
def log(logmsg: str):
    """
    Writes a log message to the specified log file.
    if log file unspecified, you have to use 'EasyLOG.logfile_path('yourpath')'
    ...
    """
    try:
        with open("EasyLOG/path.ini", "r") as f:
            path = f.readline().strip()
        now = time.strftime("%d.%m.%y - %H:%M:%S", time.localtime())
        with open(path, "a") as f:
            f.write(f"[{now}] {logmsg}\n")
    except FileNotFoundError:
        return "You have to select a path for logging, use EasyLOG.logfile_path('yourpath')"
